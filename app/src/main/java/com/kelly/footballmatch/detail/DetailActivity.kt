...
class DetailActivity : AppCompatActivity(), DetailView.View {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        presenter = DetailPresenter(this, localRepo)
    ...
    override fun showDataTeam(event: Event?) {
        if(event != null) {
            ...
            if(     event.intHomeScore.equals(null) || event.intHomeScore.equals("") ||
                    event.intAwayScore.equals(null) || event.intAwayScore.equals("") ){
                roundText = "On Going"
            }else{
                if( (event.intHomeScore!!.toInt()) > (event.intAwayScore!!.toInt())){
                    roundText = "Home Team Wins"
                }
                else if((event.intHomeScore!!.toInt() )== (event.intAwayScore!!.toInt())){
            ...
            if(!event.strHomeBadge.equals(null) && !event.strHomeBadge.equals("")) {
                Picasso.get().load(event.strHomeBadge).into(homeImg)
            }
            if(!event.strAwayBadge.equals(null)  && !event.strAwayBadge.equals("")) {
                Picasso.get().load(event.strAwayBadge).into(awayImg)
            }
            if(event.idHomeTeam.equals(null) || event.idAwayTeam.equals(null)){
